...
## Warning
Note that running this code (in particular the compilation of the C++ file) 
requires a large amount of RAM (we recommend 16GB). 
...
```
We now create a lattice for data simulation and also for model fitting/predictions. These can be the same if $\texttt{nrowcol}=\texttt{l}$ but we can choose different grid sizes for computational efficiency. 
```{r lattice}
