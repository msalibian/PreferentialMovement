...
Now we are ready to generate the data. We first simulate the latent field and then run the sampler using $\texttt{genPrefDataHybridBehav}$ which can be found in $\texttt{dataFncs.R}$. From here we will extract the data and the so-called true surface on the lattice and observed locations
